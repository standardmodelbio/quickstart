...
# GitHub raw URLs for the demo parquets (main branch; data generated by scripts/prep_mimic_demo_data.py).
...
def load_mimic_iv_demo_from_github():
    """
    Fetch MIMIC-IV demo MEDS events and labels from the quickstart repo (GitHub main).
    Events are real MIMIC-IV data; labels are generated by the prep script (see data/README.md).
    """
    print(f"\n[{STEP_DATA}/{TOTAL_STEPS}] Loading MIMIC-IV demo data from GitHub...")
    print("   -> main  (data/README.md, PhysioNet ODbL)")
...
def extract_embeddings(df, model, tokenizer, end_time=None):
    ...
    If end_time is None, uses the latest event time in df.
    """
